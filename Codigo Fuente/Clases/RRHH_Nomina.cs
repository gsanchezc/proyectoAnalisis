...
namespace Clases
{
    public class RRHH_Nomina
    {
        ...
        public DataSet buscarNomina()
        {
            ...
            {
                sql = "stp_nominaCreada";
                ParamStruct[] parametros = new ParamStruct[3];
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 0, "@idEmpleado", SqlDbType.Int, _idEmpleado);
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 1, "@mes", SqlDbType.VarChar, _mes);
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 2, "@anio", SqlDbType.Int, _anio);
                ds = cls_DAL.ejecuta_dataset(conexion, sql, true, parametros, ref mensaje_error, ref numero_error);
                if (numero_error != 0)
                {
                    MessageBox.Show(mensaje_error, "Error al obtener cadena de conexion", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public DataSet buscarNominaCompleta()
        {
            conexion = cls_DAL.trae_conexion("Soportic", ref mensaje_error, ref numero_error);
            if (conexion == null)
            {
            ...
            {
                sql = "stp_buscarNomina";
                ParamStruct[] parametros = new ParamStruct[2];
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 0, "@mes", SqlDbType.VarChar, _mes);
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 1, "@anio", SqlDbType.Int, _anio);
                ds = cls_DAL.ejecuta_dataset(conexion, sql, true, parametros, ref mensaje_error, ref numero_error);
                if (numero_error != 0)
                {
                    MessageBox.Show(mensaje_error, "Error al obtener cadena de conexion", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        public bool IngresarNuevoRegistroNomina()
        {
            ...
            else
            {
                ...
                cls_DAL.agregar_datos_estructura_parametros(ref parametros, 5, "@salario", SqlDbType.Float, _salario);
                ...
                if (numero_error != 0)
                {
                    MessageBox.Show(mensaje_error, "Error al guardar la nomina deseada", MessageBoxButtons.OK, MessageBoxIcon.Error);
